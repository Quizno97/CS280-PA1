...
using namespace std;
/*
function to check for valid flag name.(-LL,-IN,-WC,-SP)
*/
bool checkArg(string &flag)
{
    ...
    string st3="-WC";
    string st4="-SP";
    if((flag.find(st1)==string::npos)&&(flag.find(st2)==string::npos)&&(flag.find(st3)==string::npos)&&(flag.find(st4)==string::npos))
    {
        return false;
    }
    return true;
...
int getwordCount(vector<string> &WCvec) {
    int wordCount = 0;
    if (WCvec.empty()) {
        wordCount = 50;
    } else if (!(WCvec.empty())) {
        for (int i = 0; i < WCvec.size(); i++) {
            string temp = WCvec.back();
            wordCount = stoi(temp.substr(4, temp.length() - 1));
        }
    }
    return wordCount;
}
int getSpacing(vector<string> &SPvec)
{
    int spacing=0;
    if(SPvec.empty())
    {
    ...
    else if(!(SPvec.empty()))
    {
        ...
        {
            string temp=SPvec.back();
            spacing=stoi(temp.substr(4,temp.length()-1));
        }
    }
    return spacing;
}
/*
function to format line and indent.
*/
void formate(vector<string> &paragraph, int &indent, int &linelength, int wordCount, int spacing)
{
    string word,line;
    string holdstr;
...
int main(int argc, char *argv[])
{
    ...
    for(int i=0; i<flag.size();i++)
    {
        ...
        if(argument.substr(0,3)=="-WC")
        {
            WC.push_back(argument);
        }
        if(argument.substr(0,3)=="-SP")
        {
            SP.push_back(argument);
        }
    }
    for(int i=0; i<WC.size();i++){
        cout<<WC[i]<<" This is LL";
    }
    ...
    int wordCount=getwordCount(WC);
    int spacing=getSpacing(SP);
    ...
    formate(paravec, indent, linelength, wordCount, spacing);
